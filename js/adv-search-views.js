...
function _advListView(items, q) {
  ...
  items.forEach(r => {
    ...
    h += '<div class="adv-item" onclick="_advNav(\'' + r.book + '\',' + r.ch + ',' + r.v + ',\'' + (q ? q.replace(/'/g, "\\'") : q) + '\')">';
...
function _advTreeView(items, q) {
  ...
  Object.keys(groups).forEach(bk => {
    ...
    arr.forEach(r => {
      const ref = r.ch + ':' + r.v;
      const txt = _advHighlight(r.text, r.positions);
      h += '<div class="adv-item adv-tree-item" onclick="_advNav(\'' + r.book + '\',' + r.ch + ',' + r.v + ',\'' + (q ? q.replace(/'/g, "\\'") : q) + '\')">';
      h += '<span class="adv-ref">' + ref + '</span>';
      h += '<span class="adv-txt">' + txt + '</span></div>';
    });
    h += '</div></div>';
