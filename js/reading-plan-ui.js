...
function _rpShowProgress(el){
  ...
  h+=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
    ...
    <button onclick="_rpReset()" style="font-size:10px;color:var(--text3);background:none;border:1px solid var(--border);border-radius:4px;padding:2px 8px;cursor:pointer">초기화</button>
  </div>`;
  h+=`<div class="rp-progress"><div class="rp-progress-fill" style="width:${pct}%"></div></div>`;
  h+=`<div style="font-size:10px;color:var(--text3);margin-bottom:10px">${doneSet.size}/${plan.days}일 완료 (${pct}%)</div>`;
