...
function renderBookGrid(){
  ...
  h+=`<div class="bn-tab${_bnTab==='OT'?' act':''}" onclick="_bnSwitchTab('OT')">${t('ot')}</div>`;
  h+=`<div class="bn-tab${_bnTab==='NT'?' act':''}" onclick="_bnSwitchTab('NT')">${t('nt')}</div></div>`;
  ...
  if(_bnSelBook){
    ...
    h+=`<div class="bn-chapters"><div class="bn-ch-label"><span>${_bnSelBook}</span> ${t('ch.select')}</div><div class="bn-ch-grid">`;
    for(let i=1;i<=cnt;i++){
