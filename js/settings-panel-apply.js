...
// 레일 독립 강조 색상
function _stpSetRailAccent(preset){
  if(typeof ThemeSwitcher !== 'undefined' && ThemeSwitcher.setRailAccent) ThemeSwitcher.setRailAccent(preset);
  renderSettingsPanel();
}
function _stpPickRailAccentLive(hex){
  if(typeof ThemeSwitcher !== 'undefined' && ThemeSwitcher.setCustomRailAccent) ThemeSwitcher.setCustomRailAccent(hex);
}
function _stpPickRailAccent(hex){
  _stpPickRailAccentLive(hex);
  renderSettingsPanel();
}
...
function _stpResetColors(){
  if(typeof ThemeSwitcher !== 'undefined'){
    ...
    if(theme === 'light'){
      ...
      // 세피아 모드 기본값
      ThemeSwitcher.setAccent('orange');
      if(ThemeSwitcher.setCustomBase) ThemeSwitcher.setCustomBase('#ffffff');
      if(ThemeSwitcher.setBookAccent) ThemeSwitcher.setBookAccent('orange');
      if(ThemeSwitcher.setRailAccent) ThemeSwitcher.setRailAccent('orange');
      if(ThemeSwitcher.setCustomLayoutColor) ThemeSwitcher.setCustomLayoutColor('#ffffff');
    } else {
      // 다크 모드 기본값
