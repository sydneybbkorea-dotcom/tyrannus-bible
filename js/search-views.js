...
function _schListView(items, q) {
  ...
  items.forEach(r => {
    ...
    h += '<div class="adv-item" onclick="_schNav(\'' + _esc(r.b || r.book) + '\',' + r.ch + ',' + r.v + ',' + (r.nid ? '\'' + r.nid + '\'' : 'null') + ',\'' + _esc(r.type) + '\',\'' + _esc(q) + '\')">';
...
function _schRefView(items) {
  ...
  items.forEach(r => {
    // Note: r has the query attached indirectly via the closure if we passed it in earlier, but q isn't in scope here.
    // _schRefView signature doesn't have `q`. Let's wait. Or better, just focus on list/tree views where `q` is passed.
...
function _schTreeView(items, q) {
  ...
  Object.keys(groups).forEach(bk => {
    ...
    arr.forEach(r => {
      ...
      h += '<div class="adv-item adv-tree-item" onclick="_schNav(\'' + _esc(r.b || r.book) + '\',' + r.ch + ',' + r.v + ',null,\'bible\',\'' + _esc(q) + '\')">';
